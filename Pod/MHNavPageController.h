...
/*
 *  MHPageController 的缓存设置，默认缓存为无限制，当收到 memoryWarning 时，会自动切换到低缓存模式 (MHPageControllerCachePolicyLoMHemory)，并在一段时间后切换到 High .
 收到多次警告后，会停留在到 MHPageControllerCachePolicyLoMHemory 不再增长
 *
 *  The Default cache policy is No Limit, when recieved memory warning, page controller will switch mode to 'LoMHemory'
 and continue to grow back after a while.
 If recieved too much times, the cache policy will stay at 'LoMHemory' and don't grow back any more.
 */
typedef NS_ENUM(NSUInteger, MHPageControllerCachePolicy){
    MHPageControllerCachePolicyNoLimit   = 0,  // No limit
    MHPageControllerCachePolicyLoMHemory = 1,  // Low Memory but may block when scroll
    MHPageControllerCachePolicyBalanced  = 3,  // Balanced ↑ and ↓
    MHPageControllerCachePolicyHigh      = 5   // High
};

@interface MHNavPageController : UIViewController
 ...
 */
@property (nonatomic, assign) MHMenuViewStyle menuViewStyle;
 ...
 */
@property (nonatomic, assign) MHPageControllerCachePolicy cachePolicy;
 ...
 *  顶部导航栏
 */
@property (nonatomic, weak) MHMenuView *menuView;

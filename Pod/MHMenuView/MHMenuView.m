...
@interface MHMenuView () <MHMenuItemDelegate> {
    CGFloat _norSize;
    CGFloat _selSize;
    UIColor *_norColor;
    UIColor *_selColor;
...
@property (nonatomic, weak) MHProgressView *progressView;
@property (nonatomic, weak) MHMenuItem *selItem;
@property (nonatomic, strong) UIColor *bgColor;
@property (nonatomic, strong) NSMutableArray *frames;
@end
// 下划线的高度
static CGFloat const MHProgressHeight = 2.0;

@implementation MHMenuView

#pragma mark - Lazy
- (CGFloat)progressHeight {
    if (_progressHeight == 0.0) {
        _progressHeight = MHProgressHeight;
    }
...
- (void)slideMenuAtProgress:(CGFloat)progress {
    ...
    MHMenuItem *currentItem = (MHMenuItem *)[self viewWithTag:tag];
    MHMenuItem *nextItem = (MHMenuItem *)[self viewWithTag:tag+1];
    if (rate == 0.0) {
        rate = 1.0;
        self.selItem.rate = 0;
...
- (void)selectItemAtIndex:(NSInteger)index {
    ...
    MHMenuItem *item = (MHMenuItem *)[self viewWithTag:tag];
    [self.selItem deselectedItemWithoutAnimation];
    self.selItem = item;
    [self.selItem selectedItemWithoutAnimation];
...
- (void)makeStyle {
    switch (self.style) {
        case MHMenuViewStyleLine:
            [self addProgressView];
            break;
        case MHMenuViewStyleFoold:
            [self addFooldViewHollow:NO];
            break;
        case MHMenuViewStyleFooldHollow:
...
- (void)addItems {
    ...
    for (int i = 0; i < self.items.count; i++) {
        ...
        MHMenuItem *item = [[MHMenuItem alloc] initWithFrame:frame];
        item.tag = (i+kTagGap);
        item.delegate = self;
...
- (void)addProgressView {
    MHProgressView *pView = [[MHProgressView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - self.progressHeight, self.scrollView.contentSize.width, self.progressHeight)];
...
- (void)addFooldViewHollow:(BOOL)isHollow {
    MHFooldView *fooldView = [[MHFooldView alloc] initWithFrame:CGRectMake(0, 0, self.scrollView.contentSize.width, self.frame.size.height)];
...
- (void)didPressedMenuItem:(MHMenuItem *)menuItem {

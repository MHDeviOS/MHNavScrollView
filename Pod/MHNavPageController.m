...
#import "MHNavPageController.h"
#import "MHNavPageConst.h"

@interface MHNavPageController () <MHMenuViewDelegate,UIScrollViewDelegate> {
    CGFloat _viewHeight;
    CGFloat _viewWidth;
    CGFloat _viewX;
    CGFloat _viewY;
...
@implementation MHNavPageController
...
- (void)setCachePolicy:(MHPageControllerCachePolicy)cachePolicy {
...
- (void)postAddToSuperViewNotificationWithIndex:(int)index {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:MHControllerDidAddToSuperViewNotification
                                                        object:info];
}
...
- (void)postFullyDisplayedNotificationWithCurrentIndex:(int)index {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:MHControllerDidFullyDisplayedNotification
                                                        object:info];
}

// 初始化一些参数，在init中调用
- (void)setup {
    _titleSizeSelected  = MHTitleSizeSelected;
    _titleColorSelected = MHTitleColorSelected;
    _titleSizeNormal    = MHTitleSizeNormal;
    _titleColorNormal   = MHTitleColorNormal;
    
    _menuBGColor   = MHMenuBGColor;
    _menuHeight    = MHMenuHeight;
    _menuItemWidth = MHMenuItemWidth;
...
- (void)addMenuView {
    ...
    MHMenuView *menuView = [[MHMenuView alloc] initWithFrame:frame buttonItems:self.titles backgroundColor:self.menuBGColor norSize:self.titleSizeNormal selSize:self.titleSizeSelected norColor:self.titleColorNormal selColor:self.titleColorSelected];
...
- (void)resetMenuView {
    MHMenuView *oldMenuView = self.menuView;
    [self addMenuView];
    [oldMenuView removeFromSuperview];
}

- (void)growCachePolicyAfterMemoryWarning {
    self.cachePolicy = MHPageControllerCachePolicyBalanced;
...
- (void)growCachePolicyToHigh {
    self.cachePolicy = MHPageControllerCachePolicyHigh;
}
...
- (void)didReceiveMemoryWarning {
    ...
    self.cachePolicy = MHPageControllerCachePolicyLoMHemory;
    // 取消正在增长的 cache 操作
